...
type ApiConfiguration struct {
	...
	// DDoS guard settings for the Wings HTTP API.
	DDoS struct {
...
type SystemConfiguration struct {
	...
	Safety struct {
		// Enables resource anomaly guardrails in Wings.
		Enabled bool `default:"true" yaml:"enabled"`
		// Trigger threshold for "super high CPU", 500% ~= 5 cores fully utilized.
		CPUSuperCoresThresholdPercent float64 `default:"500" yaml:"cpu_super_cores_threshold_percent"`
		// Optional extra threshold for systems with many cores.
		CPUSuperAllCoresThresholdPercent float64 `default:"900" yaml:"cpu_super_all_cores_threshold_percent"`
		// Number of consecutive resource cycles required before action.
		CPUSuperConsecutiveCycles int `default:"5" yaml:"cpu_super_consecutive_cycles"`
		// Cooldown between two anomaly actions on the same server.
		ActionCooldownSeconds int `default:"300" yaml:"action_cooldown_seconds"`
		// Timeout for graceful stop before escalation/termination.
		StopTimeoutSeconds int `default:"45" yaml:"stop_timeout_seconds"`
	} `yaml:"safety"`
}
