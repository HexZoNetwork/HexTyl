...
class Node extends Model implements Identifiable
{
    ...
    public static array $validationRules = [
        ...
        'memory_overallocate' => 'required|integer|between:-1,1000',
        'disk' => 'required|numeric|min:1',
        'disk_overallocate' => 'required|integer|between:-1,1000',
    ...
    public function getConfiguration(): array
    {
        return [
            ...
            'api' => [
                ...
                'ddos' => [
                    'enabled' => (bool) config('wings_security.ddos.enabled', true),
                    'per_ip_per_minute' => (int) config('wings_security.ddos.per_ip_per_minute', 240),
                    'per_ip_burst' => (int) config('wings_security.ddos.per_ip_burst', 60),
                    'global_per_minute' => (int) config('wings_security.ddos.global_per_minute', 2400),
                    'global_burst' => (int) config('wings_security.ddos.global_burst', 300),
                    'strike_threshold' => (int) config('wings_security.ddos.strike_threshold', 12),
                    'block_seconds' => (int) config('wings_security.ddos.block_seconds', 600),
                    'whitelist' => config('wings_security.ddos.whitelist', ['127.0.0.1/32', '::1/128']),
                ],
            ],
            'system' => [
                ...
                'safety' => [
                    'enabled' => (bool) config('resource_safety.enabled', true),
                    'cpu_super_cores_threshold_percent' => (float) config('resource_safety.cpu_super_cores_threshold_percent', 500),
                    'cpu_super_all_cores_threshold_percent' => (float) config('resource_safety.cpu_super_all_cores_threshold_percent', 900),
                    'cpu_super_consecutive_cycles' => (int) config('resource_safety.cpu_super_consecutive_cycles_threshold', 5),
                    'action_cooldown_seconds' => (int) config('resource_safety.wings_action_cooldown_seconds', 300),
                    'stop_timeout_seconds' => (int) config('resource_safety.wings_stop_timeout_seconds', 45),
                ],
            ],
